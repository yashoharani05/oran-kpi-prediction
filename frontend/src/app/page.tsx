      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Select a model, enter KPI values or load a sample, then click Predict.
            </p>
          </div>
        ...
        {/* ── MODEL SELECTOR ─────────────────────────────────────── */}
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {(["random_forest", "xgboost", "lstm"] as const).map((key) => {
              ...
              const activeStyles: Record<string, string> = {
                random_forest: "bg-sky-500/20 border-sky-500/50 text-sky-300",
                xgboost:       "bg-violet-500/20 border-violet-500/50 text-violet-300",
                lstm:          "bg-emerald-500/20 border-emerald-500/50 text-emerald-300",
              };
              const active = selectedModel === key;
              return (
                <button
                  key={key}
                  onClick={() => { setSelectedModel(key); setPrediction(null); }}
                  className={`
                    px-4 py-1.5 rounded-lg text-xs font-semibold font-mono
            ...
            <span className="text-[10px] text-slate-600 font-mono">
              Switching model clears the current prediction.
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8 items-start">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-[#1e293b] border border-slate-700/50 rounded-xl p-5 flex flex-col gap-3">
              ...
              <ol className="flex flex-col gap-2.5">
                {[
                  ...
                  `The selected ${selectedModel === "random_forest" ? "Random Forest" : selectedModel === "xgboost" ? "XGBoost" : "LSTM"} model evaluates all 18 features simultaneously.`,
            ...
            {/* Model info card — updates based on selected model */}
            <div className="bg-[#1e293b] border border-slate-700/50 rounded-xl p-5">
              <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-3">
                Model Info — {selectedModel === "random_forest" ? "Random Forest" : selectedModel === "xgboost" ? "XGBoost" : "LSTM"}
              </p>
              <div className="grid grid-cols-2 gap-y-2 text-xs font-mono">
                {(selectedModel === "random_forest" ? [
                  ["Algorithm",  "Random Forest"],
                  ["Type",       "100 parallel trees"],
                  ["Train split","80% of dataset"],
                  ...
                  ["Window",     "20 rows = 5 seconds"],
                  ["Parameters", "7,073 total"],
                  ["Precision",  "~45–100%"],
                  ["Recall",     "~19–51%"],
                  ["F1 Score",   "~32–48%"],
                  ["Note",       "Needs longer sequences"],
                ]).map(([k, v]) => (
